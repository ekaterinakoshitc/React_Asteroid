...
export const Asteroids = () => {
    ...
    let [selectedDistance, setSelectedDistance] = useState(false);

    useEffect(() => {
        try {

            const response = fetch('https://api.nasa.gov/neo/rest/v1/feed?start_date=2001-01-01&end_date=2001-01-07&api_key=tZF1a0vxtZXSeqN7RiAiNc7Oar63bQBcg5djX4jp')
                .then((res) => {
                    return res.json();
                }).then((response) => {
    ...
    return(
        <div>
            ...
            <div className={styles.button_container}>
                Расстояние
                <button className={styles.distance_button} onClick={()=>setSelectedDistance(true)} >
                    в километрах
                </button>
                ,
                <button className={styles.distance_button} onClick={()=>setSelectedDistance(false)} >
            ...
            {onlyDangerous ? asteroids.filter((it) =>
                    it.isDangerous).map((item) => (
                        <AsteroidCard key={item.id} {...item} selectedDistance={selectedDistance} />))
                : asteroids.map((item) => (
                    <AsteroidCard key={item.id} {...item} selectedDistance={selectedDistance} />
                ))}
        </div>
